...
const App: React.FC = () => {
    ...
    // Local storage state (will be migrated to Firestore later)
    // The key is now dynamic based on the user's ID to keep data separate
    const userKey = user ? user.uid : 'default';
    const [businessInfo, setBusinessInfo] = useLocalStorage<BusinessInfo | null>(`businessInfo_${userKey}`, null);
    const [clients, setClients] = useLocalStorage<Client[]>(`clients_${userKey}`, []);
    const [invoices, setInvoices] = useLocalStorage<Invoice[]>(`invoices_${userKey}`, []);
    ...
    // --- Event Handlers ---
    const handleCompleteOnboarding = (name: string, rate: number) => {
        setBusinessInfo({ name, defaultInterestRate: rate });
        setClients([]);
        setInvoices([]);
